...
Kemajuan Pemrosesan Bahasa Alami (Natural Language Processing/NLP) berbasis arsitektur \textit{transformer} membuka peluang untuk membangun sistem \textit{question answering} (QA) yang mampu memahami pertanyaan dan mengekstraksi jawaban dari konteks dokumen panjang secara lebih presisi \cite{vaswani2017attention,devlin2019bert}. Tersedia model IndoBERT, termasuk varian ringan IndoBERT-Lite, menyediakan fondasi efisien untuk diadaptasi (\textit{fine-tuning}) ke tugas QA sehingga tetap terjangkau dari sisi komputasi \cite{wilie2020indobert}. Selain itu, integrasi teknik pengambilan informasi (\textit{retrieval}) sebelum inferensi, sebagaimana ditunjukkan pada paradigma \textit{retrieval-augmented}, dapat membantu sistem menjawab pertanyaan berbasis pengetahuan yang tersebar di dokumen besar \cite{lewis2020rag}, sementara studi QA atas dokumen panjang/terstruktur terus menyoroti tantangan praktis terkait segmentasi konteks, pemilihan paragraf relevan, dan keterlacakan sumber (mis. PDF/long-document QA).

Berdasarkan tinjauan tersebut, penelitian ini mengangkat tiga \textit{research gap}: (1) belum ada evaluasi terukur IndoBERT-Lite untuk QA pada dokumen PDF institusional berbahasa Indonesia dengan struktur bab/subbab; (2) belum ada perbandingan sistematis segmentasi PDF (blok/halaman vs paragraf logis) terhadap kualitas \textit{retrieval} dan jawaban; (3) pelacakan sumber (paragraf/halaman) pada jawaban QA PDF Indonesia belum distandarkan. Untuk menjawab gap ini, penelitian memanfaatkan model pralatih \texttt{Wikidepia/indobert-lite-squad} sebagai titik awal \textit{fine-tuning} dan dataset \texttt{jakartaresearch/indoqa} sebagai sumber untuk konteks-pertanyaan-jawaban berbahasa Indonesia, dengan fokus evaluasi pada skenario dunia nyata.
